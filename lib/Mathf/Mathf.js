class Mathf {
    ...
    static wrap(value, min, max) {
        if(!Type.isNumber(value)) {
            console.error("Mathf.wrap value must be a number!");
            return 0;
        }

        if(!Type.isNumber(min)) {
            console.error("Mathf.wrap min must be a number!");
            return 0;
        }

        if(!Type.isNumber(max)) {
            console.error("Mathf.wrap max must be a number!");
            return 0;
        }
    ...
    /**
     * Returns true if given value is a power of 2
     * @param {number} value value to check
     */
    static isPowerOf2(value) {
        if(!Type.isNumber(value)) {
            console.error("Mathf.isPowerOf2 value must be a number!");
            return false;
        }
