class MeshRenderer {
    ...
    recalculate() {
        var glMatrix = Matrix4.rotation(this.#actor_.rotation.x, this.#right_);
        glMatrix.multiply(Matrix4.rotation(this.#actor_.rotation.y, this.#up_));
        glMatrix.multiply(Matrix4.rotation(this.#actor_.rotation.z, this.#forward_));
        var glNormalMatrix = Matrix4.transpose(Matrix4.invert(new Matrix4().set(glMatrix)));
        glMatrix.multiply(Matrix4.scale(this.#actor_.scale));

        if(this.#mesh_ != null) {
            this.#glVertices_.length = 0;
            var vertices = this.#mesh_.vertices;
            for(var i = 0; i < vertices.length; i++) {
                vertices[i].multiplyByMatrix4(glMatrix).add(this.#actor_.position);
            ...
            for(var i = 0; i < normals.length; i++) {
                normals[i].multiplyByMatrix4(glNormalMatrix);
