class Type {
    ...
    /**
     * Returns true if the given value is an enum value of the given enumerator
     * @param {number} value value to check in enum
     * @param {Object} enum enum to check in
     */
    static isEnum(value, enumerator) {
        if(!Type.isNumber(value) || !Type.isObject(enumerator))
            return false;

        for(const num in enumerator) {
            if(enumerator[num] == value)
                return true;
        }
    ...
    static isComponent(value) {
        if(!Type.isFunction(value) || value.name === undefined || Type.isComponentInstance(value))
            return false;
        return Application.scripts.includes(value.name);
    }
