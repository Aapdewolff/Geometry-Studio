class Renderer {
    ...
    static submit(meshRenderer) {
        ...
        for(var i = 0; i < Renderer.#batches_.length; i++) {
            if(Renderer.#batches_[i].material.equals(meshRenderer.material) && (Renderer.#batches_[i].vertices.length + meshRenderer.glVertices.length) / 3 <= 65535) {
    ...
    static flush() {
        ...
        for(var i = 0; i < Renderer.#batches_.length; i++) {
            ...
            Canvas.gl.bufferData(Canvas.gl.ARRAY_BUFFER, new Float32Array(Renderer.#batches_[i].normals), Canvas.gl.STATIC_DRAW);
            ...
            Renderer.#drawCalls_++;
            Renderer.#vertices_ += Renderer.#batches_[i].vertices.length / 3;
        }
